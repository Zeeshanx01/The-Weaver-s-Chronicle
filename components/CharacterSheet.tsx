...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ player, inventory }) => {
  ...
  return (
    <div className="w-full p-4 bg-stone-900/60 border border-amber-800/50 rounded-lg animate-fade-in">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 border-b border-amber-800/50 pb-3">
        <div>
        ...
        <div className="w-full sm:w-1/3 mt-2 sm:mt-0">
            ...
            <div className="w-full bg-red-900/50 rounded-full h-4 border border-red-700/50">
                <div 
      ...
      <div>
        ...
        {inventory.length === 0 ? (
            ...
            {inventory.map((item, index) => (
              <li key={index} className="bg-stone-800/70 p-2 rounded text-center text-gray-300 border border-stone-700 capitalize">
                {item}
              </li>
            ))}
          </ul>
